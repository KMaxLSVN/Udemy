...
export class AppComponent implements OnInit {
  ...
  ngOnInit(): void {
    // this.fetchIntervalUsers(0, 9).then((data) => {
    //   console.log(data);
    // });
    this.startRecursive().then((total) => {
      console.log('Total:', total);
    });
  }

  public processUser(id: number): Promise<number> {
    let random = (Math.floor(Math.random() * 10) + 1) * 100;
  
    return new Promise<Users['id']>((resolve, reject) => {
      setTimeout(() => {
  ...
  public fetchIntervalUsers(from: number, step: number): Promise<number[]> {
    let promisesArr: Array<Promise<number>> = [];
    for (let i = from, x = (from + step); i <= x; i++) {
      promisesArr.push(this.processUser(i))
    }
    return Promise.all<number>(promisesArr);
  }

  public startRecursive(): Promise<number> {
    let index =  0;
    let currentFrom = 0;
    let sumIds = 0;
    
    function recurcive(ind: number) {
      let foo: number = this.fetchIntervalUsers(currentFrom, 9).then((response: number[]) => {
        sumIds = response.reduce((accumulator, currentValue) => accumulator + currentValue);    
        if (ind % 10 === 0) {    
          index += 10;
          currentFrom += 10;
          recurcive(index);
        } else {
          return Promise.resolve(sumIds);
        }
      }); 
      return Promise.resolve(foo);
    }

  return recurcive(index);
...
/* startRecursive()
  ...
  // public onUpdateInput(event: Event): void {
  //   const foo = (<HTMLInputElement>event.target).value;
  //   console.log(foo);
  // }
}
